...
async def editar_factura(
    ...
    if "cliente_id" in datos_actualizados:
        cliente_encontrado = sesion.get(Cliente, datos_actualizados["cliente_id"])

        if not cliente_encontrado:
            raise HTTPException(
                status_code=404,
                detail=f"Cliente con id {datos_actualizados['cliente_id']} no encontrado"
            )
